...
passwordList.forEach((item) => {
    ...
    const existing = password.split('').filter(item => item === letter).length
    const range = times.split('-')
    const min = +range[0]
    const max = +range[1]

    if (existing >= min && existing <= max) {
        console.log(`${password} is VALID, ${letter} is present ${existing} times and should have been present at least ${min} and at most ${max} times`)
    } else {
        console.log(`${password} is INVALID, ${letter} is present ${existing} times and should have been present at least ${min} and at most ${max} times`)
    }
})
